...
def print_results(hole_cards, winner_list, result_histograms):
    ...
    f = open('text.txt', 'w')
    f.write("Winning Percentages:")
    for index, hole_card in enumerate(hole_cards):
        winning_percentage = float(winner_list[index + 1]) / float_iterations
        if hole_card == (None, None):
          s = "(?, ?) : " +str(winning_percentage)
          f.write(s)
        else:
          s = str(hole_card) + ": " + str(winning_percentage)
          f.write(s)
    s = " Ties: "+ str(float(winner_list[0]) / float_iterations) + "\n"
    f.write(s)
    for player_index, histogram in enumerate(result_histograms):
        s = "Player" + str(player_index + 1) + " Histogram: \n"
        f.write(s)
        for index, elem in enumerate(histogram):
            s = str(hand_rankings[index])+ ": " + str(float(elem) / float_iterations) + "\n"
            f.write(s)
        f.write("\n")
